# Handoff: [Brief Description]

---
**Date:** [YYYY-MM-DD HH:MM]
**Author:** AI Assistant
**Status:** Handoff
**Branch:** [current branch]
**Commit:** [current commit hash]

---

## Task(s)

[Description of the task(s) being worked on, along with the status of each]

| Task | Status | Notes |
|------|--------|-------|
| [Task 1] | ✅ Complete / 🔄 In Progress / 📋 Planned | [Brief note] |
| [Task 2] | ✅ Complete / 🔄 In Progress / 📋 Planned | [Brief note] |

**Current Workflow Phase:** Research | Plan | Iterate Plan | Experiment | Implement | Validate

## Workflow Artifacts

[List all workflow documents produced or referenced during this session]

**Research Documents:**
- [research-slug.md](research-slug.md) — [Brief description of what was researched]

**Plan Documents:**
- [plan-slug.md](plan-slug.md) — [Brief description of the plan]

**Experiment Reports:**
- [experiment-slug.md](experiment-slug.md) — [Brief description of what was tested]

**Implementation Summaries:**
- [implement-slug.md](implement-slug.md) — [Brief description of what was implemented]

[Remove sections for artifact types that don't exist]

## Critical References

[2-3 most important files or documents that the next session MUST read first]

- `path/to/critical/file.ext` — [Why it's critical]
- `path/to/other/file.ext` — [Why it's critical]

## Recent Changes

[Describe recent changes made to the codebase with file:line references]

- `path/to/file.ext:lines` — [What was changed]
- `path/to/file.ext:lines` — [What was changed]

## Learnings

[Important discoveries, patterns, root causes, or insights the next session should know]

- [Learning 1 — include specific file paths where relevant]
- [Learning 2]
- [Learning 3]

## Action Items & Next Steps

[Ordered list of what the next session should do]

1. [ ] [Next action — be specific about what and where]
2. [ ] [Next action]
3. [ ] [Next action]

**Recommended Next Skill:** name the skill the receiving session should use next — one of `ai-research-workflows:researching-codebases`, `ai-research-workflows:researching-prior-art`, `ai-research-workflows:planning-implementations`, `ai-research-workflows:iterating-plans`, `ai-research-workflows:running-experiments`, `ai-research-workflows:implementing-plans`, `ai-research-workflows:validating-implementations`, `ai-research-workflows:ensuring-reproducibility`, or `ai-research-workflows:hardening-research-code`. (The matching slash commands — `/research`, `/plan`, `/implement`, `/validate`, etc. — still work as shortcuts.)

## Other Notes

[Any additional context, warnings, or useful information that doesn't fit above]

---

**Handoff created by AI Assistant on [Date]**
